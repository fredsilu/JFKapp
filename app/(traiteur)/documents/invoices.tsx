...
function getEventName(invoice: any) {
  return invoice?.eventName ?? invoice?.designation ?? "-";
}
...
function getDeliveryAddress(invoice: any) {
  return invoice?.deliveryAddress ?? invoice?.client?.city ?? "-";
}
...
type SortField = "number" | "client" | "amount" | "invoiceDate" | "createdAt";
...
const mobileSortOptions: {
  ...
  { label: "Date facture", field: "invoiceDate" },
  { label: "N° facture", field: "number" },
  { label: "Client", field: "client" },
  { label: "Montant", field: "amount" },
  { label: "Date création", field: "createdAt" },
];
...
export default function DocumentInvoicesScreen() {
  ...
  const invoices = useMemo(() => {
    const normalizedArchives = archivedInvoices.map(normalizeArchivedInvoice);
        ...
        switch (sortField) {
          ...
          case "invoiceDate":
            valueA = getDateTimestamp(
              a?.issuedAt ?? a?.invoiceDate ?? a?.documentDate,
            );

            valueB = getDateTimestamp(
              b?.issuedAt ?? b?.invoiceDate ?? b?.documentDate,
            );
            break;
        ...
        <FlatList
          ...
          renderItem={({ item }: any) => (
            <View style={styles.mobileCard}>
              ...
              <View style={styles.mobileInfoRow}>
                <Text style={styles.mobileLabel}>Date facture</Text>

                <Text style={styles.mobileValue}>
                  {formatDate(
                    item?.issuedAt ?? item?.invoiceDate ?? item?.documentDate,
                  )}
                </Text>
              </View>
              <View style={styles.mobileInfoRow}>
              ...
              <View style={styles.mobileInfoRow}>
                ...
                <Text style={styles.mobileValue}>{getPeopleCount(item)}</Text>
              </View>
        ...
        >
          <View style={styles.table}>
            <View style={styles.tableHeader}>
              ...
              <Text style={[styles.th, localStyles.colAddress]}>Adresse</Text>

              <Text style={[styles.th, localStyles.colPeople]}>#</Text>
